...
echo "检查 Black 格式..."
if black --check src/ tests/ --config pyproject.toml; then
    echo "✅ Black 格式检查通过"
else
    echo "❌ Black 格式检查失败"
    exit 1
...
echo ""
echo "检查 isort 导入排序..."
if isort --check src/ tests/ --config pyproject.toml; then
    echo "✅ isort 格式检查通过"
else
    echo "❌ isort 格式检查失败"
    exit 1
...
echo ""
echo "检查 Ruff 代码规范..."
if ruff check src/ tests/ --config pyproject.toml; then
    echo "✅ Ruff 代码检查通过"
else
    echo "❌ Ruff 代码检查失败"
    exit 1
