...
# 检�?Docker 服务状�?check_docker_status() {
    print_header "Docker 服务状�?
    docker compose ps
}
...
test_task_queue() {
    ...
    if docker compose exec -T worker celery -A src.workers.celery_app inspect stats > /dev/null 2>&1; then
...
show_help() {
    ...
    echo ""
    echo "可用命令:"
    echo "  status     查看 Docker 服务状�?
    echo "  health     健康检查（容器 + API�?
    echo "  api        API 功能测试"
    echo "  task       任务队列测试"
    echo "  logs       查看日志（最�?100 行）"
