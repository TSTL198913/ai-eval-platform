...
🛡�?**Compliance-First**: Built-in security gates and RBAC governance for LLM outputs.
...
AI-Eval-Pro 是一个面向大语言模型（LLM）的**企业级分布式评测基础设施**，提供从模型接入、评测执行、结果分析到质量门禁的完整链路。系统采�?*六层分层架构**（API �?Service �?Engine �?Domain �?Distributed �?Infra），支持 15+ 种核心评估器类型�?+ 种模型提供者，并具备企业级的分布式能力（锁、熔断、限流、幂等、队列）�?
...
- 完整的可观测性（Prometheus + Grafana + 结构化日�?+ OpenTelemetry�?
- 企业�?RBAC 权限控制与安全门�?
...
通过 `create_llm_client()` 工厂方法一键接�?5+ �?LLM Provider，支持环境变量配置与单例缓存�?
...
**支持�?Provider**：`deepseek` / `openai` / `anthropic` / `ollama` / `qwen` / `dashscope` / `stub`
...
| 类别 | 评估�?| 用�?|
|------|--------|------|
| **安全** | `SecurityEvaluator` | 注入攻击 / 越狱检�?/ 内容安全 |
| **代码** | `CodeEvaluator` / `CodeReviewEvaluator` | 代码生成与审查质�?|
| **事实�?* | `FactCheckEvaluator` / `FactualityEvaluator` | 事实核查与真实�?|
| **语义** | `SemanticEvaluator` | 语义相似度与对齐评估 |
| **问答** | `QAEvaluator` | 问答质量与相关�?|
| **记忆** | `MemoryEvaluator` | 长短期记忆能力评�?|
| **鲁棒�?* | `RobustnessEvaluator` | 提示词鲁棒性与稳定�?|
| **代理** | `MultiAgentEvaluator` | 多代理协作能�?|
| **工具** | `FunctionCallEvaluator` | 工具调用准确�?|
| **分类** | `ClassificationEvaluator` | 分类任务评估 |
| **风险** | `RiskEvaluator` | 业务风险检�?|
| **LLM裁判** | `LLMAJudgeEvaluator` | LLM-as-a-Judge 语义评估 |
| **综合** | `CompositeEvaluator` | 多维度综合评�?|
...
- **异步模式**：`POST /api/v1/evaluate/async` - 提交�?Celery 任务队列
- **批量模式**：`POST /api/v1/evaluate/sync-batch` - 批量评估
- **状态查�?*：`GET /api/v1/tasks/{task_id}` - 实时任务状�?

### 2.4 分布式能�?

- **分布式锁**（Redlock）�?防止任务重复执行
- **熔断�?*（Circuit Breaker）�?防止级联失败
- **多维限流**（Token Bucket）�?按用�?API/Worker 维度
- **幂等性检�?*（Idempotency）�?任务去重
- **优先级队�?*（Priority Queue）�?重要任务优先处理

### 2.5 企业级安全与权限

- **RBAC 权限控制**：基于角色的访问控制（Role-Based Access Control），支持 API Key 认证与权限粒度控�?
- **安全中间�?*：注入检测、内容过滤、越狱防�?
- **质量门禁**：自动评估质量阈值检查，阻止低质量模型上�?
- **审计日志**：完整的操作记录与合规追�?

### 2.6 可观测�?

- **Prometheus 指标**：`/metrics` 端点暴露评估次数、延迟、错误率
- **Grafana 仪表�?*：内�?`ai_eval_platform_ops.json` �?`ai_eval_platform_insights.json`
- **结构化日�?*：JSON 格式日志，支持链路追�?
...
| **Service** | `src/services/` | 业务编排、事务控制、跨域协�?| �?Engine / Domain |
| **Engine** | `src/engine.py` | 异常捕获、状态映射、性能计时 | �?Domain |
| **Domain** | `src/domain/` | 核心业务规则、评估器实现、模型抽�?| �?Distributed / Infra |
| **Distributed** | `src/distributed/` | 分布式原语（锁、熔断、限流、队列、幂等） | �?Infra |
| **Infra** | `src/infra/` | DB / Cache / MQ / 监控 / 安全 / RBAC | 外部资源 |

**依赖规则**（参�?[.trae/rules/arch_review.md](file:///d:/workspace/ai-eval-platform-refactor/.trae/rules/arch_review.md)）：
...
- �?评估器黑名单机制：`_EVALUATOR_BLACKLIST` 控制评估器注�?
...
| **注册表模�?* | `EvaluatorFactory._registry` | 运行时动态发现与注册 |
| **装饰器模�?* | `@EvaluatorFactory.register()` | 解耦评估器定义与注册逻辑 |
| **策略模式** | `RegisterStrategy.OVERWRITE/SKIP/ERROR` | 处理重复注册的多种策�?|
| **上下文管理器** | `get_db_session()` / `DistributedLock` | 资源生命周期自动管理 |
| **熔断�?* | `BaseEvaluator.safe_evaluate()` | 单个评估器的故障隔离 |
...
EvaluationEngine.run()
    ...
    �?     └─ 其他                   �?FAILED
    �?
    ├─ 4. 构�?EvaluationResult（case_id, status, model, adapter, latency, response�?
    �?
...
- **状态机**：`PENDING �?PASSED / FAILED / ERROR` 三态终�?
- **性能埋点**：`time.perf_counter()` 毫秒级延迟统�?
- **熔断保护**：评估器创建与执行双层熔�?
...
**注册流程**�?
1. 模块导入时触�?`auto_discover()`
2. 扫描 `src/domain/evaluators/` 下所有非排除子模�?
3. 黑名单模块被自动跳过，仅注册 15 个核心评估器
4. 每个子模�?import 时通过装饰器自动注�?
5. 启动�?`lifespan` 预热已注册评估器列表
...
**全局异常处理�?*（[src/api/server.py](file:///d:/workspace/ai-eval-platform-refactor/src/api/server.py#L144-L206)）：
- `BasePlatformError` �?400/422 + 标准化错误码
- `Pydantic ValidationError` �?400 + 字段级错�?
- `HTTPException` �?透传状态码
...
class DomainResponse(BaseModel):
    ...
    text: str | None               # 模型输出
    score: float | None            # 评分�?.0 ~ 1.0�?
    error: str | None              # 错误信息
    metadata: dict | None          # 元信�?
    data: Any | None               # 扩展数据
```
...
class EvaluationResult(BaseModel):
    ...
    model_name: str | None         # 使用的模�?
    adapter_name: str              # 评估器类�?
    response: DomainResponse       # 详细响应
    latency_ms: float              # 耗时
    error_message: str | None      # 错误描述
```

**数据库模�?*（[src/infra/db/models.py](file:///d:/workspace/ai-eval-platform-refactor/src/infra/db/models.py)）：

| �?| 关键字段 | 用�?|
|----|----------|------|
| `eval_results` | id, case_id, model_name, adapter_name, status, latency_ms, response_data, created_at | 评测结果持久�?|
...
| `GET` | `/api/v1/tasks/{task_id}` | 查询任务状�?|
| `GET` | `/api/v1/evaluators` | 列出评估�?|
| `GET` | `/api/v1/evaluators/{name}` | 评估器详�?|
| `GET` | `/api/v1/models` | 列出模型 |
| `POST` | `/api/v1/models/compare` | 模型对比 |
| `GET` | `/api/v1/records` | 评测记录列表 |
| `GET` | `/api/v1/records/{id}` | 记录详情 |
| `DELETE` | `/api/v1/records/{id}` | 删除记录 |
| `GET` | `/api/v1/dashboard/stats` | 仪表盘统�?|
| `POST` | `/api/v1/calibration/run` | 运行校准 |
| `POST` | `/api/v1/finetune/export` | 导出微调数据 |
| `GET` | `/api/v1/datasets` | 数据集列�?|
| `POST` | `/api/v1/reports/generate` | 生成报告 |
| `GET` | `/api/v1/health` | 健康检�?|
| `GET` | `/api/v1/health/detailed` | 详细健康检�?|
...
然后�?[src/api/server.py](file:///d:/workspace/ai-eval-platform-refactor/src/api/server.py) 注册�?
...
| 测试类型 | 目录 | 覆盖范围 |
|----------|------|----------|
| **单元测试** | `tests/unit/` | 核心组件、评估器、服务层�?00+�?|
| **集成测试** | `tests/integration/` | API、领域、基础设施�?7+ external�?|
| **E2E 测试** | `tests/e2e/` | 完整前后端工作流（Playwright�?|
| **可靠性测�?* | `tests/reliability/` | 分布式、并发、熔断、限�?|
| **性能测试** | `tests/performance/` | 基准测试、压力测�?|
| **安全测试** | `tests/security/` | OWASP 漏洞扫描 |
| **混沌测试** | `tests/chaos/` | 网络抖动、故障注�?|
...
- **Docker Compose**：单机快速部署（�?Prometheus / Grafana / Redis / RabbitMQ�?
- **Kubernetes**：生产级编排（见 [docker/k8s/deployment.yaml](file:///d:/workspace/ai-eval-platform-refactor/docker/k8s/deployment.yaml)）�?支持水平扩展、健康检查、优雅降�?
- **CI/CD**：GitHub Actions（[.github/workflows/](file:///d:/workspace/ai-eval-platform-refactor/.github/workflows)）�?自动化测试、构建、部署流水线
- **数据�?*：PostgreSQL（生产）/ SQLite（开发），支�?ACID 事务与连接池

### 11.2 监控

- **Prometheus**：[deploy/prometheus/prometheus.yml](file:///d:/workspace/ai-eval-platform-refactor/deploy/prometheus/prometheus.yml)
- **Grafana 仪表�?*：[grafana/dashboards/](file:///d:/workspace/ai-eval-platform-refactor/grafana/dashboards)
...
- [docs/OBSERVABILITY.md](file:///d:/workspace/ai-eval-platform-refactor/docs/OBSERVABILITY.md) - 可观测�?
- [docs/enhancement_features_guide.md](file:///d:/workspace/ai-eval-platform-refactor/docs/enhancement_features_guide.md) - 增强功能使用指南（标准指�?/ RAGAS / DeepEval / 人工标注 / 可视化）
- [.trae/documents/SYSTEM_MAPPING.md](file:///d:/workspace/ai-eval-platform-refactor/.trae/documents/SYSTEM_MAPPING.md) - 系统功能完备性矩�?
...
| **ASGI 服务�?* | Uvicorn | 生产�?ASGI |
| **任务队列** | Celery + RabbitMQ | 分布式异步任�?|
| **缓存/�?* | Redis | 分布式锁、限流、缓�?|
| **数据�?* | PostgreSQL / SQLite | 结果持久�?|
| **ORM** | SQLAlchemy 2 | 数据库访�?|
| **迁移** | Alembic | 数据库版本管�?|
| **认证** | JWT (python-jose + passlib) | 无状态认�?|
| **前端** | React 18 + TypeScript + Vite | SPA |
| **UI �?* | Ant Design 5 + Tailwind CSS | 组件与样�?|
| **状态管�?* | Zustand | 轻量状�?|
| **HTTP 客户�?* | Axios | API 调用 |
| **图表** | Recharts | 数据可视�?|
| **测试** | pytest + vitest + playwright | 单元/集成/E2E |
| **指标** | Prometheus + Grafana | 监控 |
| **追踪** | OpenTelemetry | 链路追踪 |
| **容器�?* | Docker + Docker Compose | 容器部署 |
| **编排** | Kubernetes | 生产编排 |
...
> 💡 **提示**：本 README 提供了项目的高层视角。深入实现请查阅相关模块源码�?[docs/](file:///d:/workspace/ai-eval-platform-refactor/docs) 目录下的专项文档�?
