...
def run_evaluation(test_case: dict) -> dict:
    ...
    request = EvaluationSchema(id=test_case["id"], type=test_case["type"], payload=payload)
...
def demonstrate_self_improvement():
    ...
    examples = golden_dataset_manager.get_few_shot_examples("customer_service_golden", limit=1)
    if examples:
        print("  [OK] Few-shot示例:")
        for ex in examples:
            print(f"    {ex[:100]}...")
