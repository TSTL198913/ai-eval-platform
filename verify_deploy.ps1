...
function Test-ApiFunction {
    ...
    try {
        ...
        Write-Warning "业务路由测试失败（可能需要配置 LLM）: $_"
    }
...
function Test-TaskQueue {
    ...
    # 检查 Celery Worker
    Write-Host ""
    Write-Host "检查 Celery Worker..." -ForegroundColor Gray
    
    try {
        $workerStats = docker compose exec -T worker celery -A src.worker.celery_app inspect stats 2>$null
        if ($LASTEXITCODE -eq 0) {
            Write-Success "Celery Worker 运行正常"
        }
    } catch {
        Write-Warning "Celery Worker 可能未运行: $_"
    }
}
...
function Start-FullTest {
    ...
    Write-Host "开始全面测试..." -ForegroundColor Gray
    Write-Host ""
