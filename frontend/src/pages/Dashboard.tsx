...
const Dashboard: React.FC = () => {
  ...
  const statCards = stats ? [
    { title: '总评测次数', value: stats.total_records?.toLocaleString() || '0', icon: <FileText className='w-6 h-6' />, color: 'blue', suffix: <TrendingUp className='w-4 h-4 text-green-500' /> },
    { title: '评估器类型', value: stats.evaluator_types?.length ?? 0, icon: <Brain className='w-6 h-6' />, color: 'purple' },
