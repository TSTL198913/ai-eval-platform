...
AI-Eval-Pro 是一个面向大语言模型（LLM）的**企业级分布式评测基础设施**，提供从模型接入、评测执行、结果分析到质量门禁的完整链路。系统采用**六层分层架构**（API -> Service -> Engine -> Domain -> Distributed -> Infra），支持 15+ 种核心评估器类型和多种模型提供者，并具备企业级的分布式能力（锁、熔断、限流、幂等、队列）。
...
> 📘 **新增**: 标准指标库、第三方框架适配、人工标注、可视化报告。详见 [docs/enhancement_features_guide.md](docs/enhancement_features_guide.md)
...
通过 `create_llm_client()` 工厂方法一键接入多种 LLM Provider，支持环境变量配置与单例缓存。
...
核心评估器列表见 [src/domain/evaluators/__init__.py](src/domain/evaluators/__init__.py#L67-L70)。
...
- **Grafana 仪表盘**: 内置 `ai_eval_platform_ops.json` 和 `ai_eval_platform_insights.json`
...
**核心代码**: [src/engine.py](src/engine.py)
...
**核心代码**: [src/domain/evaluators/__init__.py](src/domain/evaluators/__init__.py)
...
**核心代码**: [src/domain/models/llm_factory.py](src/domain/models/llm_factory.py)
...
**核心代码**: [src/services/evaluator_svc.py](src/services/evaluator_svc.py)
...
**核心代码**: [src/exceptions.py](src/exceptions.py)
...
**全局异常处理**（[src/api/server.py](src/api/server.py#L144-L206)）:
...
**评估请求**（[src/schemas/evaluation.py](src/schemas/evaluation.py#L27-L42)）:
...
**评估结果**（[src/schemas/schemas.py](src/schemas/schemas.py#L34-L41)）:
...
**数据库模型**（[src/infra/db/models.py](src/infra/db/models.py)）:
...
然后在 [src/api/server.py](src/api/server.py) 注册：
...
| **集成测试** | `tests/integration/` | API、领域、基础设施（57+ external） |
...
- **Kubernetes**: 生产级编排（见 [docker/k8s/deployment.yaml](docker/k8s/deployment.yaml)），支持水平扩展、健康检查、优雅降级
- **CI/CD**: GitHub Actions（[.github/workflows/](.github/workflows)），自动化测试、构建、部署流水线
- **数据库**: PostgreSQL（生产）/ SQLite（开发），支持 ACID 事务与连接池

### 11.2 监控

- **Prometheus**: [deploy/prometheus/prometheus.yml](deploy/prometheus/prometheus.yml)
- **Grafana 仪表盘**: [grafana/dashboards/](grafana/dashboards)
- **告警规则**: [deploy/prometheus/alerts.yml](deploy/prometheus/alerts.yml)

### 11.3 详细文档

- [ARCHITECTURE.md](ARCHITECTURE.md) - 架构设计细节
...
- [docs/enhancement_features_guide.md](docs/enhancement_features_guide.md) - 增强功能使用指南（标准指标 / RAGAS / DeepEval / 人工标注 / 可视化）
...
| **UI 框架** | Ant Design 5 + Tailwind CSS | 组件与样式 |
| **状态管理** | Zustand | 轻量状态 |
...
提交代码前请遵循 [.trae/rules/arch_review.md](.trae/rules/arch_review.md) 中的规则：
...
> 💡 **提示**: 本 README 提供了项目的高层视角。深入实现请查阅相关模块源码和 [docs/](docs/) 目录下的专项文档。
