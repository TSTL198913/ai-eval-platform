...
result = subprocess.run(
    ["python", "-m", "pytest", "tests/integration", "--ignore=tests/integration/test_real_data_integration.py", "--tb=no", "-q", "--no-header"],
    capture_output=True,
    text=True,
...
total_collected = 1144

print(f"=== 集成测试 FAILED 模块统计 (排除 real_data) ===")
...
with open("d:/workspace/ai-eval-platform-refactor/data/failed_modules_summary_v2.json", "w", encoding="utf-8") as f:
    json.dump(summary, f, ensure_ascii=False, indent=2)

print(f"\n结果已保存到 data/failed_modules_summary_v2.json")
